...
#define TRIGGERS 20         // 20 is a random number, just different from any specially designated number above
...
#define TRIGGER_CENTER_POS 130         // --if triggers are joined-- and both of them are in their 0 positions (not pressed), then this is the value that will be returned
...
class CoprocessorTalk {
  ...
  private:
    ...
    bool _triggersJoined = false;         // set true on call to joinTriggers()
    bool _l2Up;         // whether l2 will increase from the static position or decrease (used when triggers joined)
    bool _r2Up;
  ...
  public:
    ...
    void joinTriggers(bool l2Up = false, bool r2Up = true);
};
